...
public class bossscene1 : MonoBehaviour {
    ...
    void Start () {
        damage_rocket = 10f;damage_stream = 0.1f;
        health = 200f;point = 500f;speed_high = 5f;
        point_one = 50f;
        start_time_rocket = Time.time;
        attack = false;
        stop_boss = false;win_game = false;
        fire_work = false; high = false;
        direction = 1;
	...
	void Update () {
        if (!win_game)
        {
            ...
            if (a < 11.5f && a > 11f && !stop_boss)//planeStream di 1 doan nho thi dung lai
            {
